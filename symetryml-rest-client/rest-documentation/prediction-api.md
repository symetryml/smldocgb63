...
| **REPCA**                           | <ul><li><strong>res</strong>: 1 or 0 (1 if the returned score exceeds its threshold — see <code>repca_score_type</code>)</li><li><strong>rescore</strong>: the anomaly score. With <code>repca_score_type=hybrid</code> (default) this is <code>max(Q / Q_threshold, T² / T²_threshold)</code> and a value &gt; 1 indicates an anomaly. With <code>q</code> or <code>t2</code> it is the raw Q-statistic or Hotelling's T² and is compared against its own threshold.</li></ul>          |
